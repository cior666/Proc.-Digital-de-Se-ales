...
title('Señal de audio con las ventanas fijas de tu amigo');
...
for k = 1:size(ventanas,1)
    ...
    % DETECCIÓN FILA
    % ---------------------------
    energia_filas = zeros(1,length(f_filas));
    ...
    % DETECCIÓN COLUMNA
    % ---------------------------
    energia_cols = zeros(1,length(f_cols));
    ...
    % ---------------------------
    [~, idx_fila] = max(energia_filas);
    [~, idx_col]  = max(energia_cols);

...
% 1. Nueva señal del libro: escalón asimétrico
f = -1.*(t<0) + 1.*(t>=0);

% 2. Bases de Legendre ORTONORMALIZADAS 
phi0 = (1/sqrt(2)) * ones(size(t));
phi1 = sqrt(3/2) * t;
phi2 = sqrt(5/2) * (1.5*t.^2 - 0.5);
phi3 = sqrt(7/2) * (2.5*t.^3 - 1.5*t);

% 3. Cálculo de coeficientes por proyección ortogonal
a0 = sum(f .* phi0) * dt;
a1 = sum(f .* phi1) * dt;
a2 = sum(f .* phi2) * dt;
a3 = sum(f .* phi3) * dt;

% Reconstruccion de la señal con las bases ortonormales
f_aprox = a0*phi0 + a1*phi1 + a2*phi2 + a3*phi3;

% Error Cuadrático Total (ECT óptimo)
